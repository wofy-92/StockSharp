...
internal sealed class SmaServerStopStrategy : Strategy
{
	...
	protected override void OnOrderChanged(Order order)
	{
		...
		if (_activeStopOrder is not null &&
			order.TransactionId == _activeStopOrder.TransactionId &&
	...
	private void OnProcess(ICandleMessage candle, decimal longValue, decimal shortValue)
	{
		...
		else if (_isShortLessThanLong != isShortLessThanLong)
		{
		...
		if (Position != 0 && _activeStopOrder is null)
	...
	private void RegisterStopForPosition(decimal currentPrice)
	{
		...
		_activeStopOrder = new Order
		{
			...
			Condition = new StopOrderCondition
			{
				ActivationPrice = isLong ? currentPrice - stopOffset : currentPrice + stopOffset,
			},
			Side = isLong ? Sides.Sell : Sides.Buy,
			Volume = Position.Abs(),
			Security = Security,
			Portfolio = Portfolio,
		};
