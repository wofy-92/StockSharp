...
using StockSharp.Xaml.Charting.Controls;
...
public partial class MainWindow : Window
{
	...
	private EventSubscription CreateRealConnectorEvents()
	{
		...
		Action<long, Exception> massCancelFailed = (_, error) =>
			_uiEvents.TryAddAction(() => SetStatusIfAlive($"Real order cancellation failed: {error.Message}"));
		Action<Exception> connectionError = error =>
			_uiEvents.TryAddAction(() => SetStatusIfAlive($"Real connection failed: {error.Message}"));
	...
	private void CreateEmulationConnector()
	{
		...
		Action connected = () => _uiEvents.TryAddAction(() =>
		{
		...
		Action disconnected = () => _uiEvents.TryAddAction(() =>
		{
			if (IsCurrentEmulation(connector, generation))
				SetDisconnectedState("Disconnected");
		});
		Action<Exception> connectionError = error => _uiEvents.TryAddAction(() =>
		{
			if (IsCurrentEmulation(connector, generation))
