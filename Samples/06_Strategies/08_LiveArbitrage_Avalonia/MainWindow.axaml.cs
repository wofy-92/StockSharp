...
public partial class MainWindow : Window
{
	...
	private readonly SecurityEditor _futureSecurity;
	private readonly SecurityEditor _stockSecurity;
	private readonly PortfolioEditor _futurePortfolio;
	private readonly PortfolioEditor _stockPortfolio;
	...
	public MainWindow()
	{
		...
		_futureSecurity = this.FindControl<SecurityEditor>(nameof(FutureSecurityEditor));
		_stockSecurity = this.FindControl<SecurityEditor>(nameof(StockSecurityEditor));
		_futurePortfolio = this.FindControl<PortfolioEditor>(nameof(FuturePortfolioEditor));
		_stockPortfolio = this.FindControl<PortfolioEditor>(nameof(StockPortfolioEditor));
		...
		_futureSecurity.SecurityProvider = _context.Connector;
		_stockSecurity.SecurityProvider = _context.Connector;
		_futurePortfolio.PortfolioProvider = _context.Connector;
		_stockPortfolio.PortfolioProvider = _context.Connector;
	...
	private void OnSecurityReceived(Subscription subscription, Security security)
		=> _uiEvents.TryAddAction(() =>
		{
			...
			if (_futureSecurity.SelectedSecurity is null)
				_futureSecurity.SelectedSecurity = security;
			else if (_stockSecurity.SelectedSecurity is null && !ReferenceEquals(_futureSecurity.SelectedSecurity, security))
				_stockSecurity.SelectedSecurity = security;
		});
	...
	private void OnPortfolioReceived(Subscription subscription, Portfolio portfolio)
		=> _uiEvents.TryAddAction(() =>
		{
			...
			if (_futurePortfolio.SelectedPortfolio is null)
				_futurePortfolio.SelectedPortfolio = portfolio;
			else if (_stockPortfolio.SelectedPortfolio is null)
				_stockPortfolio.SelectedPortfolio = portfolio;
		});
	...
	private async Task StartStrategyAsync()
	{
		...
		try
		{
			...
			if (_futureSecurity.SelectedSecurity is not { } futureSecurity ||
				_stockSecurity.SelectedSecurity is not { } stockSecurity ||
				_futurePortfolio.SelectedPortfolio is not { } futurePortfolio ||
				_stockPortfolio.SelectedPortfolio is not { } stockPortfolio)
			{
				_status.Text = "Select both securities and portfolios.";
				return;
			}

			var strategy = new StockSharp.Samples.Strategies.LiveArbitrage.ArbitrageStrategy
			{
				Connector = _context.Connector,
				Security = futureSecurity,
				Portfolio = futurePortfolio,
	...
	private void SetRunningState(bool isRunning, string status)
	{
		...
		_futureSecurity.IsEnabled = !isRunning;
		_stockSecurity.IsEnabled = !isRunning;
		_futurePortfolio.IsEnabled = !isRunning;
		_stockPortfolio.IsEnabled = !isRunning;
		_status.Text = status;
	}
	...
	private async void OnClosing(object sender, WindowClosingEventArgs e)
	{
		...
		finally
		{
			...
			TryDispose(_futureSecurity);
			TryDispose(_stockSecurity);
			TryDispose(_futurePortfolio);
			TryDispose(_stockPortfolio);
