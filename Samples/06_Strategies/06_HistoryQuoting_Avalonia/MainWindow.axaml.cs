...
public partial class MainWindow : Window
{
	...
	private void AttachEvents(HistoryStrategySession session, int generation)
	{
		...
		Action<int> progressChanged = value => _uiEvents.TryAddAction(() =>
		{
			if (IsCurrent(session, generation))
				Progress.Value = value;
		});
		Action<ChannelStates> stateChanged = state => _uiEvents.TryAddAction(() =>
		{
			if (!IsCurrent(session, generation))
				return;
		...
		Action pnlChanged = () => _uiEvents.TryAddAction(() =>
		{
			if (IsCurrent(session, generation))
