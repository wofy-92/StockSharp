...
public partial class MainWindow : Window
{
	...
	private readonly IChartIndicatorElement _bands;
	...
	public MainWindow()
	{
		...
		area.Title = "Candles and Bollinger Bands";

		_candles = _chart.CreateCandleElement();
		_candles.FullTitle = "Candles";
		_bands = _chart.CreateIndicatorElement();
		_bands.FullTitle = "Bollinger Bands";
		_bands.Color = Color.CornflowerBlue;
		_bands.AdditionalColor = Color.Goldenrod;
		_bands.DrawStyle = DrawStyles.StepLine;

		_chart.AddArea(area);
		_chart.AddElement(area, _candles);
		_chart.AddElement(area, _bands);
	...
	private async Task LoadHistoryAsync(CancellationToken cancellationToken)
	{
		try
		{
			...
			{
				var value = bands.Process(candle);
		...
		catch (Exception error)
		{
			try
			{
				await Dispatcher.UIThread.InvokeAsync(
					() => Title = $"Complex Bollinger Bands - {error.Message}",
